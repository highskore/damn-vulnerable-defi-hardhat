...
describe('[Challenge] Puppet v3', function () {
  ...
  /** SET RPC URL HERE */
  const MAINNET_FORKING_URL = require('./rpc.json').RPC;
  ...
  it('Execution', async function () {
    ...
    await attackToken.approve(uniswapRouterAddress, PLAYER_INITIAL_TOKEN_BALANCE);
    const attackRouter = uniswapRouter.connect(player);

    await attackRouter.exactInputSingle(
      [attackToken.address, weth.address, 3000, player.address, PLAYER_INITIAL_TOKEN_BALANCE, 0, 0],
      {
        gasLimit: 2e7,
      }
    );
